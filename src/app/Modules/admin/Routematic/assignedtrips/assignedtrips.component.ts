...
@Component({
  selector: 'app-assignedtrips',
  templateUrl: './assignedtrips.component.html',
  styleUrls: ['./assignedtrips.component.css'],
})
export class AssignedtripsComponent implements OnInit {
  ...
  date: any;
  dailytripslist: any;
  tipContentApproveTrip: any;
  tipContentApprove: any;
  ...
  exportexcel(): void {
    let element = document.getElementById('AssignedTripsList');
    const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(element);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'AssignedTrips_Report.xlsx');
  }
